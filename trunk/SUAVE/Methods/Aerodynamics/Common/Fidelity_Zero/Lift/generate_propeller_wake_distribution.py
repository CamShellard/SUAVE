...
## @ingroup Methods-Aerodynamics-Common-Fidelity_Zero-Lift   
def generate_propeller_wake_distribution(prop,thrust_angle,m,VD,init_timestep_offset, time, number_of_timesteps ): 
    ...
    dt           = time/number_of_timesteps
    ts           = np.linspace(0,time,number_of_timesteps)
    ...
    WD        = Data()
    n         = Nr-1
    mat1_size = (m,num_prop,number_of_timesteps-1,B,n)
    ...
    mat2_size = (m,num_prop*(number_of_timesteps-1)*B*n)
    ...
    VD.Wake       = Data()
    mat3_size     = (num_prop,(number_of_timesteps-1),B,n)
    ...
    Gamma     = np.zeros((m,number_of_timesteps-1,B,n))   
    num       = int(Na/B)  
    time_idx  = np.arange(number_of_timesteps-1) 
    ...
    blade_angle_loc    = np.repeat(np.repeat(np.tile(np.atleast_2d(blade_angles),(m,1))[:,  np.newaxis, :],number_of_timesteps, axis = 1) [:, :,:, np.newaxis],Nr, axis = 3) 
    ...
    for i in range(num_prop): 
        ...
        x_pts  = np.repeat(np.repeat(x0_pts[np.newaxis,:,  :], number_of_timesteps, axis=0)[ np.newaxis, : ,:, :,], m, axis=0) 
        X_pts0 = x_pts + sx_inf   

        # compute wake contraction  
        wake_contraction = compute_wake_contraction_matrix(i,prop,Nr,m,number_of_timesteps,X_pts0)          

        y0_pts = np.tile(np.atleast_2d(r),(B,1))
        y_pts  = np.repeat(np.repeat(y0_pts[np.newaxis,:,  :], number_of_timesteps, axis=0)[ np.newaxis, : ,:, :,], m, axis=0) 
        Y_pts0 = (y_pts*wake_contraction)*azi_y  + sy_inf    

        z0_pts = np.tile(np.atleast_2d(r),(B,1))
        z_pts  = np.repeat(np.repeat(z0_pts[np.newaxis,:,  :], number_of_timesteps, axis=0)[ np.newaxis, : ,:, :,], m, axis=0)
    ...
    mat4_size = (m,num_prop,(number_of_timesteps-1),B*n)
    mat5_size = (m,num_prop,(number_of_timesteps-1)*B*n)
    mat6_size = (m,num_prop*(number_of_timesteps-1)*B*n) 
